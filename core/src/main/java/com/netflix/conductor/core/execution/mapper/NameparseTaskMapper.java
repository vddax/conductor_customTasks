...
/**
 * Maps a {@link WorkflowTask} of type {@code NAMEPARSE} to a server-side system task {@link
 * TaskModel} of type {@code NAMEPARSE}.
 *
 * <p>Modeled after {@link HTTPTaskMapper}, but does not require a {@link TaskDef} to exist.
 */
@Component
public class NameparseTaskMapper implements TaskMapper {

    public static final String TASK_TYPE = "NAMEPARSE";

    private static final Logger LOGGER = LoggerFactory.getLogger(NameparseTaskMapper.class);

    private final ParametersUtils parametersUtils;
    private final MetadataDAO metadataDAO;

    @Autowired
    public NameparseTaskMapper(ParametersUtils parametersUtils, MetadataDAO metadataDAO) {
    ...
    public List<TaskModel> getMappedTasks(TaskMapperContext taskMapperContext)
        ...
        LOGGER.debug("TaskMapperContext {} in NameparseTaskMapper", taskMapperContext);
