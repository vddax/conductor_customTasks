...
/**
 * Server-side system task for CA Address Lookup. Python parity with {@code CAAdresslookup.py}.
 *
 * <p>Expected {@code http_request.body} shape:
 *
 * <ul>
 *   <li>{@code header}: Map (e.g. contains jobId)
 *   <li>{@code request}: canonical_records (List of records each containing input.name/firm/address1/address2/lastline)
 * </ul>
 */
@Component(CaAddressLookupHttpTask.TASK_TYPE)
public class CaAddressLookupHttpTask extends HttpTask {

    public static final String TASK_TYPE = "CA_ADDRESS_LOOKUP";

    @SuppressWarnings("unused")
    public CaAddressLookupHttpTask(
    ...
    public void start(WorkflowModel workflow, TaskModel task, WorkflowExecutor executor) {
        ...
        for (int recordIdx = 0; recordIdx < canonicalRecords.size(); recordIdx++) {
            ...
            try {
                ...
                addressRecord.put("name", getNested(rec, "input", "name"));
            ...
            if (apiResults == null) {
                ...
                for (int i = 0; i < apiResults.size(); i++) {
                    ...
                    if (result.containsKey("output")) {
                        ...
                        rec.computeIfAbsent("meta", __ -> new HashMap<String, Object>());
                        ((Map<String, Object>) rec.get("meta"))
                                .put("status", "CA_ADDRESS_LOOKUP_FAILED");
                    }
                }
            }
